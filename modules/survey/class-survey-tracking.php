...
If you have questions, please contact support.', 'rm-panel-extensions'),
         ...
         WHERE approval_status = 'pending'"
                ...
                /**
                 * Get user's available surveys (not yet completed)
                 */
                public function get_available_surveys($user_id, $args = []) {
                    ...
                    $args = wp_parse_args($args, $defaults);
                    $all_surveys = get_posts($args);

                    // Filter out completed surveys
                    $completed_ids = $this->get_user_completed_survey_ids($user_id);

                    // Filter by location if FluentCRM is active
                    $available_surveys = [];
                    foreach ($all_surveys as $survey) {
                        // Skip if already completed
                        if (in_array($survey->ID, $completed_ids)) {
                            continue;
                        }

                        // Check location targeting
                        if (class_exists('RM_Panel_FluentCRM_Helper')) {
                            if (!RM_Panel_FluentCRM_Helper::matches_survey_location($user_id, $survey->ID)) {
                                // User's country doesn't match survey target
                                continue;
                            }
                        }
