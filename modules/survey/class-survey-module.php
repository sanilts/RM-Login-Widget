...
// Survey Settings
    ...
    public function render_survey_url_parameters_metabox( $post ) {
        ...
        <div class="survey-meta-field">
            ...
            <p class="description"><?php _e( 'This survey\'s unique identifier. You can use this as a parameter below.', 'rm-panel-extensions' ); ?></p>
        </div>
        ...
        <div class="survey-meta-field">
            ...
            <table id="survey-parameters-table" class="widefat" style="margin-top: 10px;">
                <thead>
                    <tr>
                        <th><?php _e( 'Data Field', 'rm-panel-extensions' ); ?></th>
            ...
            <p class="description" style="margin-top: 10px;">
                <?php _e( 'Example: If you set "Survey ID" field with variable "sid", the URL will become: https://example.com/survey?sid=' . $post->ID, 'rm-panel-extensions' ); ?>
            </p>
    ...
    public function save_meta_data($post_id, $post, $update) {
        ...
        if (isset($_POST['rm_survey_parameters']) && is_array($_POST['rm_survey_parameters'])) {
            ...
            // If no parameters are sent, delete the meta to avoid keeping old data
            delete_post_meta($post_id, '_rm_survey_parameters');
        }
     ...
     */
    /**
     * Updated handle_survey_redirect method
     * Replace the existing method in modules/survey/class-survey-module.php
     */

    /**
     * Handle Survey Redirect (Updated version with survey_id support)
     */
...
// For non-logged in users, still add survey_id and custom fields if configured
                    if (!empty($parameters)) {
                        foreach ($parameters as $param) {
                            $value = '';
...
// Add callback URL if user is logged in
                if (is_user_logged_in()) {
